...
if (Meteor.isServer) {
	eyes.defaults.stream=process.stdout;
}
...
eyes.inspector = function (options) {
    ...
    return function (obj, label, opts) {
        ...
        if (myopts.html && !myopts.stream) {
            return '<pre class="eyes" style="padding:8px;background-color:black;color:white;font-size:8px;max-width:320px;'+((myopts && myopts.styles && myopts.styles.all)?('color:'+myopts.styles.all+';'):'')+'">'+result+'</pre>';
        } else {
            return result;
        }
    };
